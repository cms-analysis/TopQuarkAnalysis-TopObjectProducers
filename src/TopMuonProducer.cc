...
//
// $Id: TopMuonProducer.cc,v 1.7.2.1 2007/08/24 13:52:27 delaer Exp $
//
...
{     
  ...
  if ( addLRValues_){
    theLeptonLRCalc_ = new TopLeptonLRCalc(setup, "", muonLRFile_);
  }
...
TopMuonProducer::findTruth(const reco::CandidateCollection& parts, const TopMuonType& muon)
{
  ...
  for(unsigned int idx=0; idx!=pairGenRecoMuonsVector.size(); ++idx){
    std::pair<const reco::Candidate*, TopMuonType*> pairGenRecoMuons;
    pairGenRecoMuons = pairGenRecoMuonsVector[idx];
...
TopMuonProducer::matchTruth( const reco::CandidateCollection& parts, TopMuonTypeCollection& muons )
{
  ...
  for( ; part != parts.end(); ++part ){
	...
	if ( ( ptRecOverGen > minRecoOnGenEt_ ) && 
      ...
      if( found == true ){
	pairGenRecoMuonsVector.push_back( std::pair<const reco::Candidate*,TopMuonType*>(&(*part), rec ) );
      }
    }
  }
}
