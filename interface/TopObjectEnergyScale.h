...
// $Id: TopObjectEnergyScale.h,v 1.1.2.2 2007/10/10 02:31:04 lowette Exp $
//
...
TopObjectEnergyScale<T>::TopObjectEnergyScale(const edm::ParameterSet& iConfig) :
  ...
  theGaussian(0)
{
  theObjects  = iConfig.template getParameter<edm::InputTag>("scaledTopObject");
  theFactor   = iConfig.template getParameter<double>       ("shiftFactor");
  theIniRes   = iConfig.template getParameter<double>       ("initialResolution");
  theFinalRes = iConfig.template getParameter<double>       ("finalResolution");
